...
function writePost(cy, text, file_image ) {
  ...
  cy.get('button[class="gh-btn gh-btn-editor gh-btn-icon only-has-icon gh-actions-cog ml3"]').click();
  cy.screenshot(file_image + i++);
}
...
function publishPost(cy, file_image ){
  cy.get('div[class="gh-publishmenu ember-view"] > div').first().click({force: true});
  cy.wait(2000)
  cy.screenshot(file_image + i++);
